...
for (var a = 0; a < category_number; a++)
{
    ...
    for (var i = 0; i < category_size; i++)
    {
        ...
        if (last_line == 1 && y_slow_down == 0 && current_text_y_placement < (110 + (slow_down_treshold * 0.5)))
            y_slow_down = 1
        draw_text(x_offset, floor(current_text_y_placement), credits_entry[a][i])
        variablenormal = i == 0
        variablenormal2 = i == (category_size - 1)
        y_current += ((line_offset + (variablenormal * header_offset)) + (variablenormal2 * section_offset))
        line_counter++
    }
}
if (y_max == -1)
