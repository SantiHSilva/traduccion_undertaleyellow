...
switch scene
{
    case 0:
        if (!audio_is_playing(mus_gunshop))
        {
            ...
            music = audio_play_sound(mus_gunshop, 20, true)
        }
    ...
    case 1:
        ...
        break
    case 2:
        if (player_npc.npc_arrived == 0 || (!scr_timer()))
        {
        }
        with (starlo_npc)
        {
    ...
    case 3:
        if ((!starlo_npc.npc_arrived) || (!scr_timer()))
            return;
        audio_play_sound(snd_locked_door, 1, false)
        timer = 30
        scene++
        break
    case 4:
