...
var particles_spawn = 0
if (sprite_index == spr_martlet_p_full_fly_begin || sprite_index == spr_martlet_g_full_fly_begin)
{
    if (floor(image_index) == 6 && no_loop_sound == 0)
    {
        audio_play_sound(snd_mart_wings_out, 20, false)
        no_loop_sound = 1
    }
...
else if (sprite_index == spr_martlet_p_full_fly_during || sprite_index == spr_martlet_g_full_fly_during)
{
        ...
        if (y > (default_y - max_y))
        {
            ...
            if (y <= (default_y - max_y))
            {
                ...
                if (!audio_is_playing(snd_mart_wind))
                    audio_play_sound(snd_mart_wind, 20, true)
            }
        }
    }
    if (fly_state == 1)
    ...
    if (fly_state == 4)
    {
        ...
        if (time_elapsed >= (time_max + 1))
        {
            ...
            {
                audio_play_sound(snd_mart_pacifistland, 20, false)
                sprite_index = spr_martlet_p_full_fly_end
            }
            else
            {
                audio_play_sound(snd_mart_genoland, 20, false)
    ...
    if (fly_state < 4)
    {
        if (floor(image_index) == 2 && no_loop_sound == 0)
        {
            particles_spawn = 1
            audio_play_sound(snd_mart_flap, 20, false)
            no_loop_sound = 1
        }
