function scr_cutscene_battle_macro_1() //gml_Script_scr_cutscene_battle_macro_1
{
    if live_call()
    ...
    switch scene
    {
        ...
        case 5:
            with (obj_micro_froggit)
            {
                ...
                {
                    var froggit_jump = audio_play_sound(snd_playerjump, 1, false)
                    audio_sound_pitch(froggit_jump, 0.5)
                    path_start(pt_micro_froggit_run, 6, path_action_stop, 0)
                }
                if (x > 640)
                {
                    instance_destroy()
        ...
        case 10:
        case 12:
            obj_macro_froggit_particle.particle_burst = 1
            if (scene == 12)
                burst_count--
            cutscene_advance()
            break
