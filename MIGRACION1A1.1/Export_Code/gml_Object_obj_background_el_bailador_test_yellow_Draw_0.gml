...
for (i = 0; i < sprite_height; i += pixel_dim)
{
    ...
    if (scroll_temp_y >= sprite_height)
        ...
        scroll_temp_y += sprite_height
    if (osc_horizontal == 1 || osc_vertical == 1)
    {
        if (osc_horizontal == 1)
        {
        ...
        if (osc_vertical == 1)
        {
            ...
            animation_disjoint_y = obj_quote_battle_ceroba_outro_4
        ...
        event_user(0)
        if (i == 0)
            var for_end = obj_quote_battle_ceroba_outro_4
        else
            for_end = (animation_disjoint_y - anim_dj_y_last)
        if (for_end < obj_quote_battle_ceroba_outro_4)
            for_end = obj_quote_battle_ceroba_outro_4
        for (k = obj_quote_battle_ceroba_outro_4; k <= for_end; k++)
        {
            for (l = 0; l <= 1; l++)
            {
