...
if (execute_normal == 1)
{
    ...
    if (execute_random == 1)
    {
        if (global.battle_enemy_name == "jandroid goosic duo" && ((global.enemy_dead + global.enemy_spared) + global.enemy_sparing) < 1)
        {
            random_number = irandom_range(0, 1)
            if (random_number == 0)
            {
                message[0] = "YEAH yeAH!!#puMP IT UP!!!!"
                message_end = 0
            }
            else if (random_number == 1)
            {
                message[0] = "goNNA woRK so#MucH HARDER!!!"
                message_end = 0
            }
        }
        if (global.battle_enemy_name == "jandroid duo" && ((global.enemy_dead + global.enemy_spared) + global.enemy_sparing) < 1)
        {
