...
{
    scr_cutscene_start()
    audio_play_sound(snd_pinkgoo_initiate, 1, false)
    liquid_active = 1
    scene = 1
}
switch scene
    ...
    case 2:
        ...
        for (var i = 0; i < array_length_1d(liquid_amount); i++)
        {
            ...
            if (liquid_amount[i] > (liquid_target[i] - 0.1) && liquid_amount[i] < (liquid_target[i] + 0.1))
            {
                ...
                {
                    liquid_noloop[i] = 1
                    var ding_sound = audio_play_sound(snd_duel_mark, 1, false)
