switch scene
{
    ...
    case 1:
        if scr_camera_move(230, 410, 2)
        {
            if (!audio_is_playing(mus_birdsofafeather))
                audio_play_sound(mus_birdsofafeather, 20, true)
            timer = 30
            scene++
        }
    ...
    case 5:
        if (!scr_timer())
            return;
        actor_martlet.is_talking = 1
        scr_text()
        with (msg)
        {
            sndfnt = 102
            message[0] = "* Recalibrando el puzzle#  de roca de lava...#  ¡Listo!"
            prt[0] = 312
        }
    ...
    case 6:
        ...
        audio_play_sound(snd_groundbreak, 1, false)
        timer = 80
        scene++
        break
    case 7:
    ...
    case 10:
        ...
        {
            sndfnt = 102
            message[0] = "* Bueno, debería aguantar#  unos cuantos usos#  al menos."
            message[1] = "* ¡Y eso es suficiente#  para mí!"
            message[2] = "* ¡Próxima parada,#  Honeydew Resort!"
            prt[0] = 323
            prt[1] = 313
            prt[2] = 312
