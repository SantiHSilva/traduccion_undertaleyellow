...
if (scene == 2)
{
    if (npc_clover.npc_arrived == 1)
    {
        npc_clover.action_sprite = 1
        npc_clover.sprite_index = spr_pl_run_down
        npc_clover.image_index = 1
        npc_clover.image_speed = 0
        with (npc_clover)
            path_start(pt_clover_jump_swing, 2, path_action_stop, 0)
...
if (scene == 4)
{
    if (keyboard_multicheck_pressed(0) || keyboard_multicheck_pressed(0))
    {
        ...
        npc_clover.image_speed = 0
        with (npc_clover)
            path_start(pt_clover_jump_off_swing, 2, path_action_stop, 0)
        audio_play_sound(snd_playerjump, 1, false)
    }
}
if (scene == 5)
