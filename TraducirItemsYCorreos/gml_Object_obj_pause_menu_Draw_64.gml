...
var xx = 52
var yy = 52
draw_sprite_ext(spr_dialoguebox, 1, xx, yy, 1.2, 0.85, image_angle, c_white, image_alpha)
draw_sprite_ext(spr_dialoguebox, 0, xx, yy, 1.1, 0.75, image_angle, image_blend, image_alpha)
...
draw_sprite_ext(spr_dialoguebox, 1, xx, (yy + bbox_yoffset), 1.2, bbox_yscale, image_angle, c_white, image_alpha)
draw_sprite_ext(spr_dialoguebox, 0, xx, (yy + bbox_yoffset), 1.1, (bbox_yscale - 0.1), image_angle, image_blend, image_alpha)
draw_set_colour(c_white)
draw_set_font(fnt_stats)
draw_text((xx - 28), (yy - 2.5), string_hash_to_newline(("LV " + string(global.player_level))))
draw_text((xx - 28), (yy + 5), string_hash_to_newline(((("PV " + string(global.current_hp_self)) + "/") + string(global.max_hp_self))))
...
for (var i = 1; i <= selection_max; i++)
{
    ...
    if ((choice[i] == "ITEM" && global.item_slot[1] == "Nothing") || (choice[i] == "CORREO" && ds_list_empty(global.mail_list)))
...
if (stats_open == 1)
{
    ...
    draw_text((xx + 57.2), (yy + 10), string_hash_to_newline(((("PV " + string(global.current_hp_self)) + " / ") + string(global.max_hp_self))))
    ...
    for (var n = 1; n <= 8; n++)
    {
        if (global.item_slot[n] != "Nothing")
        {
            if (global.item_slot[n] == "Dihydrogen Monoxide")
                ...
                draw_text((xx + 65), ((yy - 30) + (n * 15)), string_hash_to_newline("P. Amistad"))
            else
                draw_text((xx + 65), ((yy - 30) + (n * 15)), obtenerItemSpanish(string_hash_to_newline(global.item_slot[n])))
        }
    }
