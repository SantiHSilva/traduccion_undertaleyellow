...
if (damage < 1)
    damage = 1
script_execute(gml_Script_scr_controls_battle_heart_blue_down)
...
if (vsp < 20 && floating == false)
    vsp += grav
if (key_up_release && vsp < 0)
{
    vsp = 0
    floating = true
    alarm[1] = 3
}
if ((bbox_bottom + 1) >= (obj_dialogue_box_battle_transformation_any.bbox_bottom - 4))
{
    floating = false
    alarm[1] = 0
    vsp = (key_up * (-jumpspeed))
}
else if (place_meeting(x, (y + 1), obj_battle_enemy_attack_platform_parent) && (!(box_collision(x, y, obj_battle_enemy_attack_platform_parent))))
{
    floating = false
    alarm[1] = 0
    vsp = (key_up * (-jumpspeed))
}
if ((!(place_meeting(x, (y + 1), obj_battle_enemy_attack_platform_parent))) || box_collision(x, y, obj_battle_enemy_attack_platform_parent))
    hsp_carry = 0
