...
if (death_sprite == 1 && sprite_index != spr_starlo_body_dead)
{
...
if (global.turns_passed >= 17)
{
    ...
    {
        if (enemy_betrayal_noloop == 0)
        {
            enemy_sparing_old = global.enemy_sparing
            enemy_betrayal_noloop = 1
            global.enemy_sparing = 1
        }
    }
    else if (enemy_betrayal_noloop == 1)
    {
        global.enemy_sparing = enemy_sparing_old
        enemy_betrayal_noloop = 0
    }
}
if (global.enemy_dead == 1)
{
...
if (sprite_index != spr_starlo_body_dead)
{
    ...
    if (set_outro_sprite == 1 && global.turns_passed == 17)
    {
    ...
    {
        if (instance_exists(obj_heart_battle_fighting_parent) && obj_heart_battle_fighting_parent.moveable == 1)
            var attack_start = 1
        else
            attack_start = 0
        if (attack_start == 1 && sprite_index != shoot_sprite)
        {
        ...
        else if (attack_start == 0 && sprite_index != ready_sprite)
        {
...
else if (damage_disjoint_count == 0)
{
    ...
    {
        sprite_index = base_sprite
        if (global.turns_passed >= 17 && outro_sprite_noloop == 1)
        {
            sprite_index = outro_sprite
