    ...
    case 0:
        with (obj_compound_parent)
        {
            if (robot_item_is_carried == 1)
            {
                ...
                {
                    if (other.item_throw_noloop == 0 || keyboard_multicheck_pressed(1))
                    {
                        scr_cutscene_start()
                        ...
                        other.item_throw_noloop = 1
                    }
                }
                else
                    other.item_throw_noloop = 0
            }
        }
        if (keyboard_multicheck_pressed(0) && scr_interact() && screen_interact_noloop == 0)
        {
            screen_counter = 0
            screen_message = "...\nWaiting for correct\ncompounds to complete\nmixture . . ."
            if (global.route == 3)
                screen_message = "ERROR: Power low"
            screen_interact_noloop = 1
        }
        break
    case 1:
        with (compound_active)
        {
            if (robot_item_is_jumping == 0)
            {
...
if (screen_counter < string_length(screen_message))
{
    ...
    if (screen_counter == floor(screen_counter))
        audio_play_sound(snd_talk_default, 1, false)
    screen_cursor = "_"
    alarm[0] = room_speed
}
