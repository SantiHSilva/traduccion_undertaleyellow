...
switch scene
{
    ...
    case 3:
        obj_pl.direction = 0
        cutscene_advance()
        if (cutscene_follower_into_actor() == 1)
        {
    ...
    case 5:
        ...
        audio_play_sound(snd_undertale_appear, 1, false)
    ...
    case 8:
        ...
        break
    case 9:
        if (instance_exists(obj_ceroba_npc) && ceroba_is_running == 0)
        {
            ceroba_is_running = 1
            with (obj_ceroba_npc)
            {
                can_walk = 0
                action_sprite = 1
                path_start(pt_steamworks_21_ceroba_chase, 5.9, path_action_stop, 0)
        ...
        break
    case 10:
        event_music = audio_play_sound(mus_axis_chase, 1, true)
        scr_cutscene_end()
        cutscene_advance()
        break
    case 11:
        with (obj_axis_npc)
        {
            can_walk = 0
            action_sprite = 1
            path_start(pt_steamworks_21_axis_chase, axis_run_speed, path_action_stop, 0)
        }
        axis_is_running = 1
        cutscene_advance()
        break
    ...
    case 21:
        if obj_ceroba_npc.npc_arrived
        {
            with (obj_steamworks_21_locker_tall)
            {
                ...
                {
                    audio_play_sound(snd_undertale_snap, 1, false)
                    image_index = 1
                }
            }
    ...
    case 25:
        ...
        break
    case 26:
        cutscene_camera_move(obj_pl.x, obj_pl.y, 2, 1)
        with (obj_axis_npc)
        {
            action_sprite = 1
            if (path_index != pt_steamworks_21_axis_walk && other.axis_is_running == 0)
            {
                other.axis_is_running = 1
                path_start(pt_steamworks_21_axis_walk, 3, path_action_stop, 0)
            }
            if (path_position >= 1)
            {
                action_sprite = 0
    ...
    case 50:
        ...
        {
            action_sprite = 1
            if (path_index != pt_steamworks_21_axis_walk_neutral && other.axis_is_running == 0)
            {
                other.axis_is_running = 1
                path_start(pt_steamworks_21_axis_walk_neutral, 3, path_action_stop, 0)
            }
            if (path_position >= 1)
            {
...
if (ceroba_is_running == 1)
{
    with (obj_ceroba_npc)
    {
        ...
        if (path_position >= 1)
        {
            action_sprite = 0
            npc_direction = "up"
            other.ceroba_is_running = 0
        }
    }
}
if (axis_is_running == 1)
{
    with (obj_axis_npc)
    {
        if (path_position < 1)
        ...
        {
            path_position = 1
            audio_play_sound(snd_washpan_bonk, 1, false)
