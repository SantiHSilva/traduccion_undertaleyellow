...
if (enemy_dead == 1)
{
    x = xstart
    y = ystart
    instance_create(starting_point_x, starting_point_y, obj_sousborg_dead)
    layer_sequence_destroy(22)
    instance_destroy()
}
else if (enemy_spared == 1)
{
    sprite_index = spr_sousborg_spared
    x = xstart
    y = ystart
    image_speed = 0
    image_index = 0
...
else if (enemy_dead == 0 && enemy_spared == 0 && damage_disjoint_count <= 0)
{
    if (global.enemy_low_hp == 1 && global.current_hp_enemy < global.max_hp_enemy)
    {
        if (low_hp_noloop == 0)
        {
            layer_sequence_destroy(sequence_idle)
            layer_sequence_create("sequence_layer", x, y, 33)
            low_hp_noloop = 1
        }
        sprite_index = -4
    }
...
{
    sprite_index = spr_sousborg_hurt
    time_elapsed = 0
    sign_modifier = 1
