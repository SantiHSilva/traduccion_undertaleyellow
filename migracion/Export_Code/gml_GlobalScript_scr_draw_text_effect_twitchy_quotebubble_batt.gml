function scr_draw_text_effect_twitchy_quotebubble_batt() //gml_Script_scr_draw_text_effect_twitchy_quotebubble_batt
{
    ...
    for (var i = 1; i <= characters; i += 1)
    {
        ...
        if (string_lettersdigits(str_char) == "")
            draw_text(((obj_quote_bubble_battle.x + 38) + str_offset), (obj_quote_bubble_battle.y + 13), string_hash_to_newline((str_indent + str_char)))
        else
            draw_text((((obj_quote_bubble_battle.x + 38) + str_offset) + (twitchy_text_offset_x * twitchy_text_intensity)), ((obj_quote_bubble_battle.y + 13) + (twitchy_text_offset_y * twitchy_text_intensity)), string_hash_to_newline((str_indent + str_char)))
    }
}
